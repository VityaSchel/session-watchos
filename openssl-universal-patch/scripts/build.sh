...
configure() {
   ...
   elif [ "$OS" == "WatchOS" ]; then echo "${SRC_DIR}/Configure watchos-cross-$ARCH no-asm no-shared --prefix=${PREFIX}"
      ${SRC_DIR}/Configure watchos-cross-$ARCH no-asm no-shared --prefix="${PREFIX}" &> "${PREFIX}.config.log"
   elif [ "$OS" == "WatchSimulator" ]; then
      ${SRC_DIR}/Configure watchos-sim-cross-$ARCH no-asm no-shared --prefix="${PREFIX}" &> "${PREFIX}.config.log"
   fi
}
...
build()
{
   ...
   cp -R "${SCRIPT_DIR}/../openssl-${OPENSSL_VERSION}/." "${SRC_DIR}"
